...
remit_r <- adults %>%
  filter(area == "Rural") %>%
  group_by(folio) %>%
  summarize(hhsize = dplyr::first(size), remit = sum(dom_trans_monthly_inc), inc = dplyr::first(hh_tot_inc), pc_inc = dplyr::first(pc_inc),
            lfp = mean(!is.na(primary_job)) * 100, lfp_paid = mean(!is.na(lab_monthly_inc) & lab_monthly_inc > 0) * 100) %>%
  mutate(has_remit = remit != 0) %>%
  group_by(has_remit) %>%
  summarize(n = n(), hhsize = mean(hhsize) %>% round(2), remit = mean(remit) %>% round(), inc = mean(inc) %>% round(), pc_inc = mean(pc_inc) %>% round(),
            lfp = mean(lfp) %>% round(1), lfp_paid = mean(lfp_paid) %>% round(1))

knitr::kable(remit_r, caption = "Rural households by remittance")
...
knitr::kable(remit_r, caption = "Rural households by remittance")
knitr::kable(remit_u, caption = "Urban households by remittance")
```
